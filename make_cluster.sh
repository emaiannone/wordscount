...
echo ">>> Connecting to the slaves from the master through SSH..."

first_connection="sudo -u $CUSER ssh -oStrictHostKeyChecking=no MASTER exit;"
ssh -oStrictHostKeyChecking=no -i $LOCAL_PEM_AMAZON $USER_ACCESS@$MASTER "$first_connection"

for(( i=1; i<$DIM_CLUSTER; i++ ))
do
	first_connection="sudo -u $CUSER ssh -oStrictHostKeyChecking=no NODE_$i exit;"
	ssh -oStrictHostKeyChecking=no -i $LOCAL_PEM_AMAZON $USER_ACCESS@$MASTER "$first_connection"
done


dependencies="sudo apt-get update; \
	      sudo apt-get install openmpi-bin openmpi-common libopenmpi-dev lam4-dev libmpich-dev libopenmpi-dev libssl-dev cmake -y;"

echo ">>> Installing OpenMPI and OpenSSL onto the nodes"
for(( i=0; i<$DIM_CLUSTER; i++ ))
do
	ssh -oStrictHostKeyChecking=no -i $LOCAL_PEM_AMAZON $USER_ACCESS@${ip_list[$i]} "$dependencies"
done
